...
import { makeRng, clamp, pickWeighted, KEY } from './utils.js'
...
export class EventEngine {
  ...
  phoneRing(g) {
    ...
    const ev = {
      ...
      onUpdate: (et, ev2) => {
        ...
        if (g.player.pos.distanceTo(new THREE.Vector3(wx, 0, wz)) < 2.2 && g.input.justPressedAction(KEY.INTERACT)) {
          ev2.answered = true
  ...
  lightOrb(g) {
    ...
    g.dropLoot(p.x + Math.cos(angle) * d, p.z + Math.sin(angle) * d, item, count)
