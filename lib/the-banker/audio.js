// Procedural WebAudio — no asset files. Show-floor stings for the TV.
...
export const sfx = {
  ...
  smallAmount() {
    tone({ freq: 180, type: 'square', dur: 0.28, peak: 0.25 })
    tone({ freq: 140, type: 'square', dur: 0.3, at: 0.12, peak: 0.2 })
  },
  bigAmount() {
    for (let i = 0; i < 6; i++) tone({ freq: 520 + i * 130, type: 'sawtooth', dur: 0.16, at: i * 0.07, peak: 0.3 })
    noise({ dur: 1.2, at: 0.4, freq: 800, sweepTo: 4000, peak: 0.4 })
  },
  bankerRing() {
    for (let r = 0; r < 2; r++) {
      for (let i = 0; i < 8; i++) {
