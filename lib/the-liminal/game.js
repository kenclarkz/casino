...
export class Game {
  ...
  setupMobileDefaults() {
    if (!isTouch) return
    // prevent browser back-gesture from navigating away
  ...
  startSession() {
    ...
    if (isTouch) requestFullscreen(this.container)
  ...
  handleWeaponKeys() {
    ...
    if (input.justPressedAction(KEY.SWAP)) {
      this.combat.swap()
  ...
  updatePrompts() {
    ...
    const p = (t, s) => this.ui.showPrompt(this.ui.promptText(t), s)
    switch (it.kind) {
      case 'loot': {
        const def = ITEMS[it.obj.item]
        p(`[E] Take ${def ? def.name : 'Item'}`, it.obj.count > 1 ? `x${it.obj.count}` : '')
        break
      }
      case 'note':
        p('[E] Read note', 'Someone was here.')
        break
      case 'container': {
        const def = CONTAINER_TYPES[it.obj.type]
        p(`[E] Search ${def ? def.name : 'it'}`, '')
        break
      }
      case 'safe':
        p('[E] Checkpoint — rest & save', 'Health and battery restored')
        break
      case 'exit':
        if (this.isLastFloor()) {
          if (this.bossKilled) p('[E] Ascend. It is over.', '')
          else p('[E] The exit is sealed', 'The Architect must die.')
        } else {
          p('[E] Descend to the next floor', 'The stairs only go down.')
        }
        break
      case 'door':
        p('[E] Open / close door', '')
