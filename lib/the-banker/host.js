...
import {
  createGame, addPlayer, addBot, dropPlayer, applyAction, tick,
...
export function createAuthority({ code, sendTo, broadcast, onUpdate, now = () => Date.now() }) {
  ...
  function message(playerId, msg) {
    try {
      switch (msg?.t) {
        ...
        case A.ADD_BOT: {
          if (roles.get(playerId) !== 'tv') throw new Error('Only the TV can add bots')
          const g = ensureGame()
          const evs = [addBot(g)]
          pushEvents(evs); pushState()
          return
        ...
        case A.KICK: {
          if (roles.get(playerId) !== 'tv') throw new Error('Only the TV can unseat contestants')
          if (!game) return
          if (!game.players[msg.playerId]?.bot) throw new Error('Only bots can be shown the door')
          if (game.phase !== PHASES.LOBBY) throw new Error('The show already started')
          const evs = dropPlayer(game, msg.playerId)
          pushEvents(evs); pushState()
          return
        }
        case A.RESTART: {
          if (roles.get(playerId) !== 'tv') throw new Error('Only the TV can restart')
          if (game && game.joinOrder.length >= 1) {
  ...
  return { message, removePlayer, destroy, tickFor, get game() { return game } }
}
