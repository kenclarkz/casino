...
import { PHASES } from './protocol.js'
...
function handleEvents(es) {
  ...
  for (const e of es) {
    switch (e.type) {
      ...
      case 'finished': audioInit(); sfx.fanfare(); confetti(); break
    }
  }
}
    ...
    <div class="tv-lobby">
      <div>
        ...
        <div class="lobby-hint">
          ...
          2–8 contestants. The banker is waiting.
        </div>
        <div class="start-row">
          <button class="btn btn-gold" id="startBtn">START THE SHOW</button>
          <span class="min-note" id="minNote">waiting for contestants…</span>
...
function renderLobbyPlayers(s) {
  ...
  note.textContent = n < 2 ? `waiting for contestants… (${n}/8)` : `${n} seated — ready! (${n}/8)`
...
function onCaseOpened(caseId, value) {
  ...
  setTimeout(() => value >= BIG ? sfx.bigAmount() : sfx.smallAmount(), 500)
