...
function makeMats(env) {
  ...
  return {
    ...
    panel: new THREE.MeshBasicMaterial({ color: p.panel, transparent: true }),
...
export function buildFloorWorld(floor) {
  ...
  for (const d of floor.doors) {
    ...
    pivot.add(doorMesh)
    group.add(pivot)
    const doorObj = { id: d.id || `${d.x}-${d.y}`, x: x, z: z, pivot, doorMesh, open: d.open, targetOpen: d.open, rot: (d.rot || 0) + Math.PI / 2 }
