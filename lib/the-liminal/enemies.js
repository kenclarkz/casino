...
export class Enemy {
  ...
  animate(dt) {
    ...
    if (f.lengthSq() > 0.001) {
      ...
      this.mesh.rotation.y += lerpAngle(targetYaw - this.mesh.rotation.y, 0, Math.min(1, dt * 6))
    }
    this.mesh.position.copy(this.pos)
    if (this.hitFlash > 0) {
...
export class BossArchitect {
  ...
  update(dt) {
    ...
    this.mesh.rotation.y += lerpAngle(targetYaw - this.mesh.rotation.y, 0, Math.min(1, dt * 4))
    this.mesh.position.copy(this.pos)
