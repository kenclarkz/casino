...
// Bots think for a beat before acting, so a solo show keeps its TV rhythm.
// The delay is drawn from the seeded rng to keep replays deterministic.
const BOT_MIN_MS = 900
const BOT_MAX_MS = 2400
const BOT_NAMES = ['BOLTS', 'COGWORTH', 'SERVO', 'SPANNER', 'WIDGET', 'CLANKTON', 'FLIPSY', 'DIAL-TONE']
...
export function createGame({ code, seed, now }) {
  ...
  return {
    ...
    botAt: 0,           // when the bots take their next turn (their own clock)
    botFor: null,       // phase the bot clock was armed for
    turnIndex: 0,
    deadline: 0,
    log: [],
  }
...
// Seat a house robot. TV-only in practice; the lobby is the only doorway.
export function addBot(state) {
  if (state.phase !== PHASES.LOBBY) throw new Error('Game already started')
  if (state.joinOrder.length >= MAX_PLAYERS) throw new Error('Show is full (8 max)')
  const used = new Set(state.joinOrder.map(id => state.players[id]?.name))
  const name = BOT_NAMES.find(n => !used.has(n)) ?? `BOT-${state.joinOrder.length + 1}`
...
export function startGame(state, now) {
  ...
  if (seated.length < MIN_PLAYERS) {
    throw new Error(`Need at least ${MIN_PLAYERS} contestant${MIN_PLAYERS === 1 ? '' : 's'}`)
  }
...
// Every value the outside world can still influence: your own mystery case
// plus all unopened communal cases. Drives both the banker's math and the
// bots' deal-or-no-deal judgement (public information only — no peeking).
function openPoolValues(state, playerId) {
  const pool = []
  const p = state.players[playerId]
...
export function bankerOffer(state, playerId) {
  const pool = openPoolValues(state, playerId)
...
// ---------------------------------------------------------------------------
// bots — house contestants for solo shows
// ---------------------------------------------------------------------------

function botWorkPending(state) {
  switch (state.phase) {
    case PHASES.PICKING:
      return state.joinOrder.some(id => {
        const p = state.players[id]
        return p?.bot && p.caseId == null
      })
    case PHASES.OPENING: {
      const cur = state.players[state.joinOrder[state.turnIndex]]
      return !!cur?.bot
    }
    case PHASES.OFFER:
      return Object.entries(state.offers).some(([id, o]) => state.players[id]?.bot && !o.answered)
    case PHASES.TWIST:
      return (state.twistPendingIds || []).some(id => {
        const p = state.players[id]
        return p?.bot && p.twistChoice == null
      })
    default:
      return false
  }
...
// Closed cases hide their values from everyone, so any pick is equally
// informed — the bot just picks with panache (random, like the AFK autopilot).
function botOpenChoice(state) {
  const free = unopenedCommunal(state)
  return { t: A.OPEN_CASE, caseId: state.rng.pick(free).id }
}

// Deal when the banker's number beats a healthy slice of the public average.
// Early offers never clear the bar; late ones usually do — like the show.
function botDealChoice(state, playerId) {
  const pool = openPoolValues(state, playerId)
  const ev = pool.reduce((a, b) => a + b, 0) / Math.max(1, pool.length)
  const bar = 0.62 + state.rng.next() * 0.26
  const take = state.offers[playerId].amount >= ev * bar
  return { t: take ? A.DEAL : A.NO_DEAL }
}

function runBots(state, now, events) {
  switch (state.phase) {
    case PHASES.PICKING: {
      for (const id of state.joinOrder) {
        const p = state.players[id]
        if (!p?.bot || p.caseId != null) continue
        const free = Object.values(state.cases).filter(c => !c.owner)
        if (!free.length) break
        try { events.push(...applyAction(state, id, { t: A.PICK_CASE, caseId: state.rng.pick(free).id }, now)) } catch { /* race */ }
      }
      break
    }
    case PHASES.OPENING: {
      ...
      if (pid && state.players[pid]?.bot && unopenedCommunal(state).length) {
        try { events.push(...applyAction(state, pid, botOpenChoice(state), now)) } catch { /* ignore */ }
      }
      break
    }
    case PHASES.OFFER: {
      for (const id of Object.keys(state.offers)) {
        ...
        if (!off || !state.players[id]?.bot || off.answered) continue
        try { events.push(...applyAction(state, id, botDealChoice(state, id), now)) } catch { /* ignore */ }
        if (state.phase !== PHASES.OFFER) break // round resolved mid-loop
      }
      break
    }
    case PHASES.TWIST: {
      for (const id of state.twistPendingIds || []) {
        const p = state.players[id]
        if (!p?.bot || p.twistChoice != null) continue
        try {
          events.push(...applyAction(state, id, { t: A.TWIST_CHOICE, swap: state.rng.next() < 0.5 }, now))
        } catch { /* ignore */ }
        if (state.phase !== PHASES.TWIST) break // twists applied mid-loop
      }
      break
    }
  }
...
export function tick(state, now) {
  ...
  // Bots run on their own short fuse so a solo show keeps its pace; humans
  // keep the full countdown. A stale clock never survives a phase change.
  if (state.botAt && state.botFor !== state.phase) state.botAt = 0
  if (!state.botAt && botWorkPending(state)) {
    state.botAt = now + state.rng.int(BOT_MIN_MS, BOT_MAX_MS)
    state.botFor = state.phase
  }
  if (state.botAt && now >= state.botAt) {
    state.botAt = 0
    runBots(state, now, events)
  }
