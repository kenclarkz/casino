...
// One router for every message addressed to THIS tab, whichever transport
// delivered it — including errors the authority bounces back to the TV.
function routeMsg(msg) {
  ...
  else if (msg.t === 'error') showError(msg.m)
}
...
function boot() {
  ...
  if (transportMode() === 'ws') {
    ...
    auth = createAuthority({
      ...
      now: () => Date.now(),
      sendTo: (pid, msg) => {
        if (pid === TV_ID) return routeMsg(msg) // the host talks to itself too
  ...
  $('main').innerHTML = `
    <div class="tv-lobby">
      <div>
        ...
        <div class="start-row">
          <button class="btn btn-gold" id="startBtn" disabled>START THE SHOW</button>
